...
error SprkHausHolder__ProposalRejected();
error SprkHausHolder_ClaimedNotPossible();
error SprkHausHolder_NotEnoughFunds();

/**
 * @title SprkHausCollab - A Collaborative Crowdfunding NFT Smart Contract
 * @dev This contract enables the creation of crowdfunding campaigns as NFTs. 
 * Each NFT represents a unique crowdfunding opportunity with milestones.
 */

contract SprkHausHolder is Context, ERC721A , ReentrancyGuard {
    ...
    modifier onlyOperator() {
        require(
            ...
            "SprkHausHolder: User is not authorized"
        );
        _;
    }
    
    modifier onlyProposalCreator {
        require(_msgSender() == proposalCreator,"SprkHausHolder: User is not proposal creator");
        _;      
    }

    modifier onlyWhenProposalIsNotActive {
        require(
            block.timestamp < fundingActiveTime,"SprkHausHolder: Funding has been intiated , action cannot be performed"
        );
        _;
    }
    modifier onlyWhenNotPaused {
        require(
            pause == false ,"SprkHausHolder: Funding is paused!"
        );
        _;
    }
    ...
    constructor(
        ...
        require(proposalDetails.length == 4,"SprkHausHolder: Invalid Proposal Input");
        ...
        require(contractAddr.length == 2,"SprkHausHolder: Invalid Contract Input");
    ...
    function _transferFunds(address from,address to,uint256 amount) private  returns(bool){
        ...
        require(value >= amount, "SprkHausCollab: Not Enough Funds!");
        bool success;
        if (from == address(this)) {
            success = token.transfer(to, amount);
            require(success, "SprkHausCollab: Transfer failed");
        } else {
    ...
    function intiateProposalFunding()external onlyProposalCreator {
        require(
            ...
            "SprkHausHolder: Proposal cannot be intiated"
        );
        _unpause();
    }
    ...
    function stake(uint256 amount) external onlyProposalCreator onlyWhenProposalIsNotActive{
        stakingAmount = (crowdFundingGoal * 20) / 100;
         require(
            ...
            "SprkHausCollab: Funds should be equal to staking amount"
        );
        require(
            isCreatorStaked == false,
            "SprkHaus: Proposal Creator already staked"
        );
    ...
    function mintTicket(uint256 quantity)external returns (uint256 prevQuntity,uint256 currentQuantity) {
        require(
            ...
            "SprkHausCollab : Proposal is being rejected"
        );
        require(
            totalSupply() + quantity <= maxSupply,
            "SprkHausHolder: Amount cannot be minted!"
        );
        require(block.timestamp >= fundingActiveTime && block.timestamp < fundingEndTime,"SprkHausHolder: Funding cannot be done"); 
        /// if the creator didn't staked the proposal will be rejected and users can claimback
        if(isCreatorStaked == false){
                _proposalRejection();
                revert SprkHausHolder__ProposalRejected();        
        }
    ...
    function withdrawFunds(address wallet, uint256 amount) external onlyProposalCreator onlyWhenNotPaused nonReentrant{
        require(
            ...
            "SprkHausHolder: Amount to be collected more than staked"
        );
         require(
            fundsInReserve >= amount,
            "SprkHausCollab: Process cannot proceed , more than reserve fund"
        );
    ...
    function claimback() external nonReentrant returns(uint256 ,bool) {
       ...
       uint256 refundValue;
       require(nftBalance > 0,"SprkHausHolder: User cannot claimback");
       require(refundStatus[_msgSender()] == false,"SprkHausHolder: Refund is already claimed!");
       if(fundingEndTime < block.timestamp && fundsInReserve != crowdFundingGoal){   
           ...
           return (refundValue,refundStatus[_msgSender()]);
            ...
            revert SprkHausHolder_ClaimedNotPossible();
       }
    }   
    ...
    function yieldSubmission() external onlyProposalCreator{
        require(isYieldReturned == false,"SprkHausHolder: Action cannot be proceeded");
        ...
        if(balanceOfCreator >=  yeildToBeRecieved){
            ...
            if(deficit <= stakingAmount){
                ...
                revert SprkHausHolder_NotEnoughFunds();
            }
       }
    ...
    function unStake() external onlyProposalCreator  returns(uint256) {
        require(
            ...
            "SprkHausHolder: User cannot withdraw funds");
        require(stakingAmount > 0,"SprkHausHolder: Not Enough Staking Funds");
        if(fundsInReserve == 0){
            require(isYieldReturned == true,"SprkHausHolder: Yield is not submitted");
        }
        isCreatorStaked = false;
    ...
    function tokenURI(
        ...
        require(_exists(tokenId), "SprkHausHolder: Non-Existent Asset");
        return baseURI;
    }
