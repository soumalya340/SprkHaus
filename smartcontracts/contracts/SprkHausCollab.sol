...
error SprkHausCollab__ProposalRejected();
error SprkHausCollab_ClaimedNotPossible();
 ...
 */
contract SprkHausCollab is Context, ERC721Enumerable, ReentrancyGuard {
    ...
    modifier onlyOperator() {
        require(
            ...
            "SprkHausCollab: User is not authorized"
        );
        _;
    }
    ...
    modifier onlyProposalCreator() {
        require(
            ...
            "SprkHausCollab: User is not proposal creator"
        );
        _;
    }
    ...
    modifier onlyWhenProposalIsNotActive() {
        require(
            ...
            "SprkHausCollab: Funding has been intiated , action cannot be performed"
        );
        _;
    }
    modifier onlyWhenNotPaused() {
        require(pause == false, "SprkHausCollab: Funding is paused!");
        _;
    }
    ...
    constructor(
        ...
        require(
            ...
            "SprkHaus: Invalid Proposal Input"
        );
        ...
        baseURI = _baseURI;
        require(contractAddr.length == 2, "SprkHaus: Invalid Contract Input");
    ...
    function intiateRejection() external {
        require(
            ...
            "SprkHausCollab: Rejection cannot be done"
        );
        _proposalRejection();
    ...
    function stake(uint256 amount) external onlyProposalCreator {
    ...
    function mintTicket() external returns (uint256) {
        ...
        require(
            block.timestamp >= fundingActiveTime &&
                block.timestamp < fundingEndTime,
            "SprkHausCollab: Funding time has been passed"
        );
        if (isCreatorStaked == false) {
            _proposalRejection();
            revert SprkHausCollab__ProposalRejected();
        }
        require(
            fundsInReserve < crowdFundingGoal,
            "SprkHausCollab: Funding goal has been reached"
        );
        _transferFunds(_msgSender(), address(this), salePrice);
        fundsInReserve += salePrice;
        uint256 tokenIdTracker = counter++;
        uint256 currentTokenID = tokenIdTracker;
    ...
    function withdrawFunds(
        ...
        require(
            ...
            "SprkHausCollab: Amount to be collected more than staked"
        );
        require(
            fundsInReserve >= amount,
            "SprkHausCollab: Process cannot proceed , less than reserve fund"
        );
        fundsInReserve -= amount;
        _pause();
    ...
    function claimback(
        ...
        require(
            ...
            "SprkHaus: User is not the token owner"
        );
        require(
            refundStatus[tokenId] == false,
            "SprkHaus: Refund is already claimed!"
        );
        if (
            ...
            revert SprkHausCollab_ClaimedNotPossible();
        }
    }
    ...
    function unStake() external onlyProposalCreator returns (uint256 amount) {
        require(
            ...
            "SprkHaus: User cannot withdraw funds"
        );
    ...
    function stakeRefundByOperator()
        external
        onlyOperator
    ...
    function setFundingStartTimeOperator(uint256 time) external onlyOperator {
        fundingActiveTime = block.timestamp + time;
    }

    function setFundingEndTimeOperator(uint256 time) external onlyOperator {
        fundingEndTime = block.timestamp + time;
    }
    ...
    function tokenURI(
        ...
        require(_exists(tokenId), "SprkHausCollab: Non-Existent Asset");
        return baseURI;
    }
